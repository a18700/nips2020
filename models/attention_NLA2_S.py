...
class AttentionConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, bias=False, rpe=False, scale=False, return_kv=False, layer=0):
        ...
        self.scoring = nn.Sequential(*[nn.Conv2d(self.l_channels//self.groups, 1, kernel_size=1, bias=bias) for i in range(self.groups)])
    ...
    def forward(self, x, abs_x, idx):
        ...
        ''' 1.5. Addressing '''
        #TODO scaling coefficient - GD's idea
        if self.scale:
        ...
        ''' 'self-attention graph pooling style '''
        for i in range(self.groups):
            out_i = out[:,i,:,:].unsqueeze(2) # B, G, C//G, N -> B, C//G, N -> B, C//G, 1, N
            score = F.tanh(self.scoring[i](out_i)) # B, C//G, 1, N -> B, 1, 1, N 
            if i == 0:
                group_score = score
            else:
